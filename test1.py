...
class User:
    ...
    def pay_with_balance(self, target, amount, note):
        if self.balance < amount:
            raise PaymentException('inssufienct balance')
        
        self.balance -= amount
        payment = payment(self, amount, self.username, target, note)
        target.add_to_balance(amount)
        self.activity_log.append(payment)
        target.activity_log.append(payment)
        pass
    ...
    def render_feed(self, feed):
        for event in feed:
            if isinstance(event, Payment):
                print(event.actor.username + " paid " + event.target.username + )
        pass
...
if __name__ == '__main__':
    unittest.main()
# TT/VN - Sr. Python with AWS Engineer - Job4516 - Juan Cruz Cordoneda (juancordoneda@gmail.com)